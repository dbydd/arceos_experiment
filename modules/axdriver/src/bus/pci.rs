...
impl AllDevices {
    pub(crate) fn probe_bus_devices(&mut self) {
        ...
        let base_vaddr = phys_to_virt(axdtb::find_dtb_node("pci-host-ecam-generic").map(|node|node.reg.first().map(|range|range.start).unwrap().into()).unwrap_or(axconfig::PCI_ECAM_BASE.into()));
