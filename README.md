...
本项目需要参赛队伍**将ArceOS移植到飞腾派开发板**，实现**UART串口通信**，并且实现**I2C驱动**或**USB驱动**。
...
* 工作量有多大？
    1. 我们查阅飞腾官方的手册并逆向其sdk源代码，在硬件规格部分保密的情况下成功的完成了系统移植
    2. 移植工作包括但不止于：肉眼读汇编代码进行模拟，在无仿真器/gdb的情况下对代码插桩输出进行定位错误点。
    3. 以上任务都是在串口输出没有移植完毕的情况下同步进行的，我们同时移植好了串口驱动和操作系统，换句话来的，我们在没有完整的串口输出的情况下移植系统
